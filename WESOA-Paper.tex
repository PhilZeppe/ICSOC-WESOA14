...
\label{sec:intro}
  ...
  \textbf{Contribution. }
   %
   In the described motivational scenario the MP needs a platform to communicate with and deploy ad-hoc assembled, dynamic teams of human and machine service units in order to execute, often unforeseeable, complex collaboraton patterns. The SmartSociety platform was designed to cater to these requirements, while \mdl{}, one of its core components, is the module responsible for managing all communicational aspects of it. \mdl{} fulfills the following requirements characteristic of most service buses:
     \begin{inparaenum}[a) \itshape]
        %
        \item \textit{Heterogeneity} -- supporting various types of communications channels (protocols) between the platform and service units as well as among service units/collectives, transparently to the platform user.
        %
        ...
        %
        %\item \textit{Interpretation} -- message interpretation service for peers.
        %
        \item \textit{Scalability} -- ability to handle large number of intermittently available service units.
      \end{inparaenum}

	However, in addition to these requirements, the distinguishing novelty of \mdl{} is the native support for virtualizing \text{collectives}: \mdl{} hides the complexity of communication with a dynamic collective as a whole and passing of instructions from the CAS programming model to it, making it a first-class entity; communication with a human or a machine element is transparent to the programmer; humans can participate in diffrent collectives concurrently, acting as different service units with diffrent delivery and privacy policies. These properties make \mdl{} suitable for  



A middleware acting as a smart society service bus will automatically route instructions to
the right virtualized computing elements. The routing algorithms will utilize several description information and
non-functional properties associated with humans/machines and matching techniques developed in other WPs.
Furthermore, the middleware also performs initiating and provisioning virtualized services on-demand as well as
to integrate with existing systems providing and hosting machines (e.g., cloud systems) and humans (e.g., social
networks). 

The result of this task would be techniques and middleware that allow other components to request,
communicate and invoke individual human-based and machine-based computing elements in a similar way
...
\textbf{Paper Organization. }
  ...
  \textbf{Other Functionalities.}
    ...
    \mdl{} provides Delivery Policies an different levels: message, peer and collective level. 
	%The policy on message level defines some basics like acknowledged messages, the policies on peer level define how messages are sent over the various adapters (e.g., by using all available adapters or just by the preferred one). On the collective level the delivery policy defines the communication strategy with the members of the collective and how failed communication attempts are indicated (e.g., a message being sent to all members could not be delivered to one specific peer). 
	Each level defines some rules/strategies on how to communicate and how/when communication failures are detected.
	Those policies can be combined and will be executed sequentially, whereas the collective level is the highest and the message level is the lowest level.
	Furthermore peers can define some privacy policies which can restrict or even prohibit the sending of messages in the case of a privacy policy violation (e.g., a peer does not want to be contacted on weekends).
...
\label{sec:impl}
  ...
  \mdl{} uses MongoDB\footnote{\url{http://www.mongodb.org}} as a database system for its various subsystems. Depending on the usage of the middleware, either an in-memory or dedicated database instances of MongoDB can be used. To decouple the execution of the HDA-CAS platform and the communication we use Apache ActiveMQ\footnote{\url{http://activemq.apache.org}} as the message broker.
  	 ...
  	 \mdl{} can be started calling \textit{initializeSmartCom()} on an instance of the class \textit{at.ac.tuwien.dsg.smartcom.SmartCom}. The methods \textit{getCommunication()}, \textit{getMessageInfoService()}, and \textit{getQueryService()} provide the services and classes that allow the interaction with \mdl{}. The various subsystems and the whole system can be build using Apache Maven\footnote{\url{http://maven.apache.org/}}. The APIs are provided in the module 'api' of the prototype implementation. Additional information on the implementation, usage and the source code can be found on GitHub\footnote{https://github.com/tuwiendsg/SmartCom}.
  ...
  %\subsection{Use-Case Walkthrough}
    ...
    The use-case starts by SmartSociety application notifying peers that their participation is needed (Fig.~\ref{fig:use-case}, \texttt{1}\degree) by sending a message to the \texttt{\href{https://github.com/tuwiendsg/SmartCom/blob/master/messaging/src/main/java/at/ac/tuwien/dsg/smartcom/messaging/MessagingAndRoutingManagerImpl.java}{Messaging and Routing Manager}} which will initialise the routing. Some peers expressed in their profiles the preference for being notified by SMS, others by email. To send an SMS the \texttt{\href{https://github.com/tuwiendsg/SmartCom/blob/master/messaging/src/main/java/at/ac/tuwien/dsg/smartcom/messaging/MessagingAndRoutingManagerImpl.java}{Messaging and Routing Manager}} reads the phone number of a peer from its profile and hands it to the \texttt{\href{https://github.com/tuwiendsg/SmartCom/blob/master/adapter/src/main/java/at/ac/tuwien/dsg/smartcom/manager/am/AdapterManagerImpl.java}{Adapter Manager}} which instantiates and executes the SMS adapter. The \texttt{\href{https://github.com/tuwiendsg/SmartCom/blob/master/api/src/main/java/at/ac/tuwien/dsg/smartcom/adapter/OutputAdapter.java}{peer adapter}} sends the \texttt{\href{https://github.com/tuwiendsg/SmartCom/blob/master/api/src/main/java/at/ac/tuwien/dsg/smartcom/model/Message.java}{message}} using the most cost-efficient mobile operator. Those peers that prefer to be contacted through email will be sent an email using a stateless email adapter through an external mail service. This preference can be set using a \texttt{\href{https://github.com/tuwiendsg/SmartCom/blob/master/api/src/main/java/at/ac/tuwien/dsg/smartcom/model/DeliveryPolicy.java}{Delivery Policy}}.
    %
