...
\hyphenation{crowd-sour-cing middle-ware}
...
\begin{figure}[H] %place exactly here, from 'float' package
\centering
...
\begin{figure}[t] %place exactly here, from 'float' package
\centering
\includegraphics[width=#1\columnwidth]{figures/#2}
\caption{#3}
\label{fig:#2}
...
\email{\{zeppezauer,oscekic,truong,dustdar\}@dsg.tuwien.ac.at}
\texttt{http://dsg.tuwien.ac.at}}
...
\label{sec:intro}
  ...
  \section{Motivation \& Requirements}
    ...
    \label{sec:intro:context}
      ...
      %At the same time, the the collaboration patterns help the peers to initialize and better self-control the collaboration. A SmartSociety application developer can indicate which patterns to use.
      %, while leaving the on-the-site organization to the peers themselves.
      %
      A collaborative pattern consists of the following elements:
          ...
          %\begin{LaTeXdescription}
            ...
            %Specification of channels in principle depends on the imposed topology. SSP cannot provide communication functionality between peers, nor between peers and the SSP. Therefore, all the communication should be performed using existing tools (e.g., mail, twitter, Skype). An adapter may be needed when the external tools is used to communicate with SSP. 
    ...
    \label{sec:intro:scenario}
      ...
      % Let us consider a municipality which wants to leverage SmartSociety platform to build and deploy a distributed application for emergency response. A SmartSociety application is developed and deployed on the platform. Citizens register to participate and either download a dedicated peer app on their devices, or participate (even concurrently) via web interface from arbitrary devices. By registering, the citizens (i.e., peers from now on) become eligible to participate and to be invited to join different possible collectives belonging to this SmartSociety application.
      % %
      % We will first describe a general scenario to help better understand one possible usage context.\footnote{The scenario was chosen as a representative of a class of similarly complex tasks requiring active human participation. Other examples may include: crowd-powered smart-city management, locals-driven tourist guidance or car-sharing scenarios.} 
      % Later in the paper \hl{add Section number} will consider a particular use-case belonging to this scenario that will be used 
      % %throughout the remainder of the paper 
      % as a running example to showcase \mdl's functionality.


      % %\emph{General scenario}---
      ...
      % %
      % The peer-to-peer collaboration is preferably executed with the support of well-known external software tools (peers). Therefore, each collective is provided with an external virtual collaboration and coordination environment. For example, a collective is given a URL to access a Google map on which they can enter markers (road under water, house damaged, people inside), credentials for a cloud-based storage service (e.g., Dropbox/ownCloud) where they can upload the GPS-annotated pictures of the objects, as well as a Twitter hashtag with which they can annotate tweets they can freely use to communicate between each other.
...
%PREDICTIVE MAINTENANCE
    ...
    \label{sec:intro:reqs}
      ...
      \begin{enumerate}
        ...
        %
        %\item \emph{Interpretation}---message interpretation service for peers.
        %
        \item \textbf{Scalability}---ability to handle large number of intermittently available service units.
      \end{enumerate}
%\hl{PUT HERE A LINE WHY EXISTING SOLUTIONS ARE NOT GOOD}
      %%Note that  \mdl{} does not need to provide an environment for peers to actually execute the task, nor does it strive to be the only possible communication channel for peers participating in the task execution.
      
%\hl{go through paper and put HDA-CAS instead of SmartSociety where appropriate}
...
\label{sec:middleware}
  ...
  %\hl{Philipp+Ognjen 2p}. Present the Middleware design on high level explaining architectural choices to fulfill REQs.
  ...
  \label{sec:middleware:routing}
    ...
    %There are three filtering steps based on message properties: receiver ID, message type and message subtype. Each step will determine the route based on the property that is handled in the particular step and a mapping of that property to identifiers, calculate an intersection of the current result and the received list of previous steps and forward the resulting list to the next step. If one property is empty or null, the appropriate step will be skipped. The result is a list of routes that matches the given message.
	  % 
    ...
    \begin{itemize}
      \item \emph{Adapter Handler}: managing adapter instance lifecycle. It handles the following adapter types:  
            \begin{itemize}%[\itshape i)]
              ...
              %The adapter is thread-safe by design since only one thread executes the adapter at a time.
              %
              ...
              %Usually only one thread executes the adapter but with increasing load further threads might execute the adapter and send messages in parallel. Hence that the stateless peer adapter is not thread safe and should be designed in a way that no synchronisation is required.
              %
              ...
              %They are also created by applications running in the Task Execution Engine and will listen on a communication channel for incoming messages (e.g., email push notification).
            \end{itemize}
...
\label{sec:relwork}
	...
	%SmartCom presents a novel way of communication within HDA-CAS, its subsystems and peers. We are not aware of any research projects that deal with such a system but there have been publications of other FoCAS members which are related and dedicated to the same purpose.
  ...
  %a direct communication between peers would be possible in general but is not considered as the default behaviour.
